...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchMealsOfDay() {
    try {
      ...
      mealsOfDayList.innerHTML =
        '<p style="color:#888;">Failed to load meals of the day.</p>';
    }
  }
  ...
  function renderMealsOfDay() {
    if (!mealsOfDay.length) {
      mealsOfDayList.innerHTML =
        '<p style="color:#888;">No meals of the day set.</p>';
      return;
    }

    mealsOfDayList.innerHTML =
      '<div class="recent-orders"><table><thead><tr><th>Image</th><th>Name</th><th>Price</th><th>Quantity</th><th>Action</th></tr></thead><tbody>' +
      mealsOfDay
        .map((meal) => {
          ...
          if (item.priceOptions && item.priceOptions.length > 0) {
            priceDisplay = item.priceOptions
              .map((p) => `Ksh ${Number(p.price).toLocaleString()}`)
              .join(" / ");
          }
  ...
  function renderMenuMeals() {
    if (!menuItems.length) {
      menuMealsList.innerHTML =
        '<p style="color:#888;">No menu meals added yet.</p>';
      return;
    }

    const menuMealsListHTML =
      '<div class="recent-orders"><table><thead><tr><th>Image</th><th>Name</th><th>Category</th><th>Price</th><th>Quantity</th><th>Action</th></tr></thead><tbody>' +
      menuItems
        .map((meal) => {
          let priceDisplay = `Ksh ${Number(meal.price).toLocaleString()}`;
          if (meal.priceOptions && meal.priceOptions.length > 0) {
            priceDisplay = meal.priceOptions
              .map((p) => `${p.size}: Ksh ${Number(p.price).toLocaleString()}`)
              .join("<br>");
          }
  ...
  window.removeMenuMeal = async function (id) {
    if (!confirm("Are you sure you want to remove this meal from the menu?"))
      return;

    try {
      const { token, adminType } = getAdminAuth();
      const response = await fetch(
        `https://aticas-backend.onrender.com/api/menu/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  window.editMenuMeal = function (id) {
    ...
    document
      ...
      .setAttribute("data-existing", meal.image || "");
  ...
  window.editMealOfDay = function (id) {
    ...
    document.getElementById("mealName").value =
      meal.menuItem?.name || meal.name || "";
    modMealQuantity.value = meal.quantity ?? 10;
    document.getElementById("mealImage").value = "";
    document
      .getElementById("mealImage")
      .setAttribute("data-existing", meal.menuItem?.image || meal.image || "");
    addMealModal.style.display = "flex";
    addMealModal.querySelector(".modal-content").style.background = "yellow";
  };
  ...
  addMealForm.onsubmit = async (e) => {
    ...
    const priceOptionItems =
      priceOptionsWrapper.querySelectorAll(".price-option-item");
    ...
    const processMeal = async (imageData) => {
      try {
        // Add imageData to mealData if provided
        if (imageData) {
